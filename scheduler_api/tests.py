...
class Schedule(APITestCase):
    ...
    def test_create_schedule(self):
        ...
        mapping = {'None': None, 'Schedule' : 'song', 'Not None' : 'Stand UP - Product Discussion'}
        ...
        for _, row in df_test_cases.iterrows():
            with self.subTest(params=row):
                # Pre-cleanup 
                scheduler_schema.models.Schedule.objects.all().delete()
                User.objects.all().delete()
                if row['startTime'].lower() == 'future':
                    start_date_time = (timezone.now() + datetime.timedelta(seconds=50)).strftime('%Y-%m-%dT%H:%M:%SZ') 
                else:
                    start_date_time = (timezone.now() - datetime.timedelta(seconds=50)).strftime('%Y-%m-%dT%H:%M:%SZ') 

                if row['endTime'].lower() == 'future':
                    end_date_time = (timezone.now() + datetime.timedelta(seconds=50)).strftime('%Y-%m-%dT%H:%M:%SZ') 
                else:
                    end_date_time = (timezone.now() - datetime.timedelta(seconds=50)).strftime('%Y-%m-%dT%H:%M:%SZ') 
                ...
                if row['users'].lower() == 'available':
                    ...
                    user = User.objects.create_user(username='john',
                                 email='jdoe@abc.com',
                                 password='something123')
                    schedule = scheduler_schema.models.Schedule.objects.create(title="Important Meeting",
    ...
    def test_list_schedule(self):
        ...
        schedule1 = scheduler_schema.models.Schedule.objects.create(title="Important Meeting",
                                                                    start_date_time=start_date_time,
                                                                    end_date_time=end_date_time,
                                                                            )
        schedule1.users.add(user1) 
        
        schedule2 = scheduler_schema.models.Schedule.objects.create(title="Important Meeting",
                                                                    start_date_time=start_date_time,
                                                                    end_date_time=end_date_time,
                                                                            )
    ...
    def test_update_schedule(self):
        ...
        for _, row in df_test_cases.iterrows():
            with self.subTest(params=row):
                ...
                schedule_main = scheduler_schema.models.Schedule.objects.create(title="Some Important Meeting",
                                                            start_date_time=start_date_time,
                                                            end_date_time=end_date_time,
                                                            )
                ...
                response = self.client.patch(url, headers= {'Content-Type': 'application/json'}, data=payload, format='json')
                self.assertEqual(
                    response.status_code, row['Expected'],
                    msg={'url': url, 'request_payload': payload, 'response': response.json()}
                )
